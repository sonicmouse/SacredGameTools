...
#include "../memdc.h" // memdc.h is in parent directory for now -MOUSE
...
void CPPDumpCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (m_bPressedLButton)
	{
		...
		if (IsWindow(GetSafeHwnd()) && GetNotify())
		{
			...
			//if (lpnm.iFirstAddr != lpnm.iLastAddr) // taken out so we get an update when select changes -MOUSE
			//{
				lpnm.hdr.hwndFrom = m_hWnd;
				lpnm.hdr.idFrom   = GetDlgCtrlID();
...
CString CPPDumpCtrl::FormatingString(int nIndex, int nValue, int nLength /* = 0 */)
{
	...
	case PPDUMP_BAR_AREA_ASCII:
		//Ascii (Added in block of high chars... -MOUSE)
		if ((nValue <= 0x20 || nValue > 0x7E) && m_chSpecCharView)
...
{
	SetColor (PPDUMP_COLOR_DATA_FG, RGB(0,0,0), FALSE);
	SetColor (PPDUMP_COLOR_DATA_BK, RGB(255,255,255), FALSE);
	SetColor (PPDUMP_COLOR_DATA_CHANGE_FG, RGB (255, 0, 0), FALSE);
	SetColor (PPDUMP_COLOR_EDIT_FG, RGB (0, 0, 0), FALSE);
	SetColor (PPDUMP_COLOR_EDIT_BK, RGB (127, 127, 127), FALSE);
	SetColor (PPDUMP_COLOR_EDIT_ERR_FG, RGB (255, 255, 255), FALSE);
	SetColor (PPDUMP_COLOR_EDIT_ERR_BK, RGB (255, 0, 0), FALSE);
	SetColor (PPDUMP_COLOR_CARET_BK, RGB (180,180,210), FALSE);
	SetColor (PPDUMP_COLOR_ADDRESS_FG, RGB (0, 0, 200), FALSE);
	SetColor (PPDUMP_COLOR_SEPARATORS, RGB(127,127,127), FALSE);
	SetColor (PPDUMP_COLOR_TEXT_HEADER, RGB(127,127,127), FALSE);
	SetColor (PPDUMP_COLOR_MOUSE_TRACK, RGB(10,36,106), bRedraw);
