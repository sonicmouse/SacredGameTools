...
void CTreeCtrlEx::OnPaint() 
{
	...
	CRect myRect; GetClientRect(myRect);
	CMemDC dc(&odc,&myRect);
	...
	{		
		...
		else if (m_mapColorFont.Lookup(hItem, cf))
		{
			...
			//memDC.TextOut(rect.left + 2, rect.top + (GetItemHeight() - 16)/2 + 1, sItem);
...
void CTreeCtrlEx::OnDeleteitem(NMHDR* pNMHDR, LRESULT* pResult){
