...
void CSITFileDlg::OnFileNameChange(){
	...
	if(pathname.GetLength()){
		BeginWaitCursor();
		CSITFile sit;
		if(sit.DecompileSitFile(pathname)){
			m_Img.SetImage(sit.GetItemPicture());
			SetDlgItemText(IDC_EDIT_NAMEZ, *sit.GetItemName());
			SetDlgItemText(IDC_EDIT_DESCR, *sit.GetItemDesc());
			SetDlgItemText(IDC_EDIT_AUTHZ, *sit.GetItemAuthor());
			SetDlgItemInt(IDC_EDIT_MINLVLZ,sit.GetItem()->GetItemData()->buf[0x14]);
			SetDlgItemInt(IDC_EDIT_LVLZ,sit.GetItem()->GetItemData()->buf[0x19]);
			SetDlgItemInt(IDC_EDIT_IDZ,*sit.GetItem()->GetItemDescId(),FALSE);
			GetDlgItem(IDC_BUTTON_VIEWDUMP)->EnableWindow(TRUE);
			CString invSize; invSize.Format("Size: %ux%u; Slotted: %u", sit.GetItem()->GetItemInvSize()->cx, sit.GetItem()->GetItemInvSize()->cy, sit.GetItem()->GetSlottedArray()->GetSize());
	...
	SetDlgItemText(IDC_EDIT_MINLVLZ,"");
	SetDlgItemText(IDC_EDIT_LVLZ,"");
