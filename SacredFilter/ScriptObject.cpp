...
{
	_tcscpy(m_szError,TEXT("Unable to create an instance of the script control."));
	...
	m_szError[0]=0;
	m_szScriptPath[0]=0;
	_tcscpy(m_szLanguage, LANGUAGE_VBSCRIPT);
...
bool CScriptObject::GetMethodHasRetVal(int index){
	LPCTSTR szName = GetNameAt(index);

	bool bResult = false;
	if (m_pScript != NULL && _tcslen(szName))
	{
		...
		try{
			...
			for(long index=1; index <= count; index++)
			{
				...
				if(_tcsicmp(name,szName)==0){
					if(pIProcPtr->GetHasReturnValue() == VARIANT_TRUE){
						bResult = true;
					}
				}
				pIProcPtr = NULL;
...
long CScriptObject::GetMethodParamCount(int index){
	LPCTSTR szName = GetNameAt(index);
				...
				if (ReadFile(hFile, pbytes, dwSize, &dwRead, NULL))
				{
					try{
						if (m_pScript)
						{
							...
							m_pScript->AddCode( strCode );
							GetMethodsName();
							_tcscpy(m_szScriptPath,szFilename);
							bResult = true;
						}
					}
					catch(...)
