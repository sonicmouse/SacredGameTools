...
BOOL CALLBACK DialogProc(HWND hwndDlg,UINT uMsg, WPARAM wParam, LPARAM lParam){
	...
	switch(uMsg){
		case WM_INITDIALOG:
			...
			{
				SetDlgItemInt(hwndDlg,IDC_EDIT_LEVEL,*(unsigned short*)&g_ucHeroData[0x3E3],FALSE);
				SetDlgItemInt(hwndDlg,IDC_EDIT_EXP,*(int*)&g_ucHeroData[0x399],TRUE);
			}
			break;
		case WM_COMMAND:
			wmId = LOWORD(wParam);
			...
			switch(wmId){
				case IDOK:
					{
						*(unsigned short*)&g_ucHeroData[0x3E3] = (unsigned short)GetDlgItemInt(hwndDlg,IDC_EDIT_LEVEL,NULL,FALSE);
						*(int*)&g_ucHeroData[0x399] = GetDlgItemInt(hwndDlg,IDC_EDIT_EXP,NULL,TRUE);
					}
...
void __stdcall seaseven_library_details(char* szLibraryName /* MAX 255 chars */,
	...
	strcpy(szLibraryName, "Edit Level and Experience");
...
void __stdcall seaseven_plugin_entry(HWND hWndParentApp /* HWND to SeaSeven application */,
	...
	if(rc==IDOK){
		MessageBox(hWndParentApp,"Successfully updated hero's experience and level!","Success...",MB_OK|MB_ICONINFORMATION);
	}
}
